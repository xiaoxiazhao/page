...
QHeader.Userlog = {
	...
	init: function(){
		...
		var node =  this.logbefore.select('.entry')[0];
		if(!node){ return; }
	...
	drawUserinfo: function(data){
		this.lock = false;
		if(data['userName'] == 'null'){ //exception
			this.logbefore.show();
			this.logafter.hide().update('');
			this.uid = 0;
			return this;
		}
		var loghtml = '<div class="entry" id="qheader_username"><a href="http://'+UC_DOMAIN+'/u/home/" target="_blank"><i class="ico-user-l2"></i>'+truncate(data['userName'], 5, '...')+'</a></div>';
		this.logafter.update(loghtml);

		//更新上传面板下的空间链接
		if(data['userEncodeId']){
			var uphref = 'http://'+UC_DOMAIN+'/u/'+data['userEncodeId'];
			$('qheader_upload_userspace').href = uphref;
		}
	...
	getUserinfo: function(){
		...
		loghtml = '<div class="entry" id="qheader_username"><a title="'+username+'" href="http://'+UC_DOMAIN+'/u/home?type=watch_timeline" target="_blank"><i class="ico-user-l2"></i>'+truncate1(username, 10, '...')+'</a></div>';
		this.logafter.update(loghtml);

		//更新上传面板下的空间链接
		if(userencoid){
			var uphref = 'http://'+UC_DOMAIN+'/u/'+userencoid;
			$('qheader_upload_userspace').href = uphref;
		}
...
QHeader.Looking = {
	...
	bindList: function(){
		...
		var remove = function(o){
			...
			//如果是节目的话  则还需要从剧集列表中删除数据
			if(d.showid){
				 dl = 'http://yus.navi.youku.com/playlog/show/delete.json?'
					+ 'token=' + token
					+ '&v=' + d.videoid
					+ '&fid=' + d.folderid
					+ '&shid=' + d.showid
			...
			chkempty();
			if(hz){
				var hzurl = "http://hz.youku.com/red/click.php?tp=1&cp="+hz+"&cpp=1000768";
				var img3 = new Image();
				img3.src = hzurl;
			}
...
QHeader.Notice = {
	...
	timer: null,
	itemDetails:{
				'comment_reply':{'label':'','count':0,'cpp':4004471,'url':'/u/comments/gt_reces_ct_2'},
                 ...
                 'subscribe':{'label':'好友更新','count':0,'url':'/u/home?type=subscribe','cpp':4006336},
	...
	update: function(){
		...
		if(uid){
			...
			//var url = 'http://10.10.69.150/notice/js_notify.json?__ap=' + ap + '&__callback=' + cb;
			addScript(url, null, true);
		}else{
			this.polled = true;
		}
	...
	drawNum: function(info){
		...
		QHeader.NoticeDetail.update(info,total2);
 		// 获取订阅相关数量
		if ( info.notice['is_subscribe'] == 1 ) {
			var subscribe_total = info.notice.subscribe;
		} else {
			var subscribe_total = 0;
		}
		QHeader.Subscribe.drawSubscribeNum( subscribe_total );
		return this;
	},
    drawUserPannelNum: function( total ){
            var total = parseInt(total) || 0 ;
            if ( total <= 0 ){
                return ;
            }
            var UserNewsPop = document.getElementById('yk-userlog-newspop');
            if ( total > 0 ) {
                var UserNewsPopNum = document.getElementById('yk-userlog-newspop-num');
                UserNewsPopNum.innerHTML = total;
                UserNewsPop.style.display = "block";
            } else {
                UserNewsPop.style.display = "none";
            }
    },
	start: function(){
...
QHeader.NoticeDetail = {
		...
		for(var item in QHeader.Notice.itemDetails){
			...
			if(info.notice[item] > 0){
				...
				if(['statuses_comments','comment_reply','video_reply','comment_mentions'].include(item)){
					...
					if(['subscribe','star_statuses','show_statuses','upvideo_succeed','upvideo_failed','upvideo_blocked','upvideo_unblocked','fee_expire','dms','hudong'].include(item)){
	...
	otherStatusLabel: function(){
		...
		if(!this.request_detail){
			...
			var url = 'http://'+ DOMAIN_NOTICE + '/notice/js_detail.json?__ap=' + ap + '&__callback=' + cb;
			//var url = 'http://10.10.69.150/notice/js_detail.json?__ap=' + ap + '&__callback=' + cb;
			addScript(url, null, true);
		}
	...
	noticeItemMerge:function(part1,part2){
		...
		if(len1 == 0 && len2 == 0){
            ...
            if($('tab_msg').style.display != 'block'){
	...
	bulidLabel:function(ret,attr,index){
		...
		var span_failed ='<span class="hint"><i class="ico-notice">发布失败</i></span>';
                var span_subscription = '<span class="hint"><i class="ico-subscription"></i></span>';
		var label = null;
		...
		switch(attr){
			...
			case 'dms':
				...
				break;
			case 'hudong':
				var url = ret.notice[attr][0].url;
				var msg = ret.notice[attr][0].msg;
				var url = "http://hz.youku.com/red/click.php?tp=1&cp=4009301&cpp=1000814&url="+decodeURIComponent(url);
				var label = '<a attr="hudong" href="' + url + '" target="_blank">' + msg + '</a>';
				if ( this.showHudong === false ) {
					hz('4009332','1000814');	//展示的时候发送的统计代码
				}
				this.showHudong = true;
				break;
			default:
				var label = '';
		}
	...
	parseDetailItem:function(ret){
		...
		if(ret.notice){
			for(var p in ret.notice){
				if(['subscribe','star_statuses','upvideo_succeed','upvideo_failed','upvideo_blocked','upvideo_unblocked','fee_expire','dms','hudong'].include(p) && ret.notice[p].length > 0){//-show_statuses
					...
					if(ret.notice[attr].length >= 1){
						if (len == 1){
							if(video_type == 85){
								...
								var t ='节目';
								if(update_num){
									var trun_len = 19-t.length;
									var action = truncate(video_title,trun_len,'...')+'更新到'+update_num;
								}else{
									var trun_len = 19-t.length;
									var action = truncate(video_title,trun_len,'...')+'有更新';
								}
							}
						}else{
						...
						if(len == 1 || tmp.length ==1){
							...
							var html='<span class="updateinfo"><a target="_blank" attr="show_statuses" href="http://'+UC_DOMAIN+'/u/home?type=favorite&sub=show">'+action+'</a></span><span class="status" id="m_status"></span><span class="action">'+see+'<a class="flink" id="see" target="_blank" href="http://'+UC_DOMAIN+'/u/home?type=favorite&sub=show" attr="show_statuses">查看详情 </a></span>';
						}else{
							var html='<span class="updateinfo"><a attr="show_statuses" target="_blank" href="http://'+UC_DOMAIN+'/u/home?type=favorite&sub=show">'+action+'</a></span><span class="action"><a id="see" attr="show_statuses" target="_blank" href="http://'+UC_DOMAIN+'/u/home?type=favorite&sub=show">查看详情 </a></span>';
						}
                        if ($("notice_show").style.display != 'block'){ // 避免重复统计
                            Nova.addScript("http://hz.youku.com/red/click.php?tp=1&cp=4008198&cpp=1000404&"+Math.random());
                        }
	...
	clearSelf:function(el){
		if(el) {
			...
			if(['comment_all','star_statuses','show_statuses','subscribe','upvideo_succeed','upvideo_failed','upvideo_blocked','upvideo_unblocked','fee_expire','statuses_mentions','followers','video_reply','comment_reply','comment_mentions','dms', 'sysmsg','hudong'].include(type)){
...
QHeader.Subscribe = {
	...
	init: function() {
		...
		this.dp = new Dropmenu({
			...
			'fire': 'hover'
                ...
                // 元素展示和用户登录有关
                var logelemt = $('qheader_logbefore');
                if ( !logelemt.visible() ){
                    node.up().show();
                } else {
                    node.up().hide();
	...
	hide: function(){
        ...
        getSubscribeNum : function( real ){
            var subscribe_num = $('old_subscribe_num').value;
            if ( !real && subscribe_num > this.maxnum ) {
                subscribe_num = '99+';
            }
            return subscribe_num;
        },
        setSubscribeNum : function ( num ) {
            if ( num != parseInt(num) ) {
                return false;
            }
            $('old_subscribe_num').value = num;
        },
        getNewSimbol : function() {
            var simbol = $('new_subscribe_simbol').value;
            return simbol;
        },
        setNewSimbol : function ( si ){
            if ( si !== 0 && si !== 1 ) {
                return;
            }
            var simbol = $('new_subscribe_simbol');
            simbol.value = si;
        },
        checkNeedQuery : function(){
            var simbol = this.getNewSimbol();
            if ( simbol == 1 ) {
                return true;
            }
            var content = this.dp.panel.innerHTML;
            if ( content == "" ) {
                return true;
            }

        },
        drawSubscribeNum: function( num ){
            var num_real = parseInt(num);
            if ( num_real > this.maxnum ){
                num = this.maxnum+'+';
            } else {
                num = num_real;
            }
            // ajax登录，不刷新不会展示的bug, 只用登录会展示这个方法，所以直接show
            var subscribe_block = $('qheader_subscription').up('.yk-subscription');
            if ( subscribe_block.visible() === false ){
                subscribe_block.show();
            }
            var old_num = this.getSubscribeNum( true );
            var subscribe_num = this.dp.node.down('.yk-subscription-num');
            if ( old_num == 'NaN' || old_num != num_real ) {
                // 写入需要更新list
                this.setNewSimbol(1);
                this.setSubscribeNum(num_real);
            }
            if ( num == 0 ){
                subscribe_num.hide();
            } else {
                subscribe_num.innerHTML = num;
                subscribe_num.show();
            }
        },
	getSubscribeList: function(){
        ...
        hz(4009107,1000798);
        // 真正需要更新的情况
        if ( this.checkNeedQuery() ) {
            this.dp.update('<div style="text-align:center;padding:33px 0;height:16px;overflow:hidden;">'+ LOADING +'</div>');
            var t = +new Date();
            var url = 'http://'+ DOMAIN_NC + '/index_QHeaderJSONP?function[]=subscribe&callback[]=QHeader.Subscribe.drawSubscribeList&t=' + t;
            // 实际滑动统计
            hz(4009141,1000798);
            addScript(url, null, true);
        }
	},
	drawSubscribeList: function( json ){
                // 这返回json
...
//FX
	(function () {
		this.FX = function (b, c, d, e, f, g) {
			this.el = a.get(b), this.attributes = c, this.duration = d || .7, this.transition = e && e in FX.transitions ? e : "easeInOut", this.callback = f || function () {
				}, this.ctx = g || window, this.units = {}, this.frame = {}, this.endAttr = {}, this.startAttr = {}
		}, this.FX.transitions = {
			...
			start: function () {
				...
				this.getAttributes(), this.duration = 1e3 * this.duration, this.time = (new Date).getTime(), this.animating = !0, this.timer = setInterval(function () {
					...
					a.time + a.duration > b ? (a.elapsed = b - a.time, a.setCurrentFrame()) : (a.frame = a.endAttr, a.complete()), a.setAttributes()
				}, 10)
			}, ease: function (a, b) {
				...
				for (attr in this.startAttr)if (this.startAttr[attr]instanceof Array) {
					this.frame[attr] = [];
					for (var a = 0; this.startAttr[attr].length > a; a++)this.frame[attr][a] = this.ease(this.startAttr[attr][a], this.endAttr[attr][a])
		...
		var a = {
			...
			return function (b) {
				if (a[b])return a[b];
				var c = b.split("-"), d = c[0];
				if (c.length > 1)for (var e = 1, f = c.length; f > e; e++)d += c[e].charAt(0).toUpperCase() + c[e].substring(1);
				return a[b] = d
			}
		}(), c = function () {
		...
		if (f || (f = d * .3 * 1.5), !e || Math.abs(c) > e) {
			e = c;
			var g = f / 4
		} else var g = f / (2 * Math.PI) * Math.asin(c / e);
		return 1 > a ? -.5 * e * Math.pow(2, 10 * (a -= 1)) * Math.sin((a * d - g) * 2 * Math.PI / f) + b : .5 * e * Math.pow(2, -10 * (a -= 1)) * Math.sin((a * d - g) * 2 * Math.PI / f) + c + b
	}, FX.transitions.backIn = function (a, b, c, d, e) {
		return e === void 0 && (e = 1.70158), c * (a /= d) * a * ((e + 1) * a - e) + b
	}, FX.transitions.backOut = function (a, b, c, d, e) {
		return e === void 0 && (e = 1.70158), c * ((a = a / d - 1) * a * ((e + 1) * a + e) + 1) + b
	}, FX.transitions.backBoth = function (a, b, c, d, e) {
		return e === void 0 && (e = 1.70158), 1 > (a /= d / 2) ? c / 2 * a * a * (((e *= 1.525) + 1) * a - e) + b : c / 2 * ((a -= 2) * a * (((e *= 1.525) + 1) * a + e) + 2) + b
		...
		return 1 / 2.75 > (a /= d) ? c * 7.5625 * a * a + b : 2 / 2.75 > a ? c * (7.5625 * (a -= 1.5 / 2.75) * a + .75) + b : 2.5 / 2.75 > a ? c * (7.5625 * (a -= 2.25 / 2.75) * a + .9375) + b : c * (7.5625 * (a -= 2.625 / 2.75) * a + .984375) + b
...
//        QHeader.Subscribe.getSubscribeList();
//    }
//}
